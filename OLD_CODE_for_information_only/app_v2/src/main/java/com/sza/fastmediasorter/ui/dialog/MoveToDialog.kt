...
class MoveToDialog(
    ...
    private fun moveToDestination(destination: MediaResource) {
        ...
        if (totalSize > 1024 * 1024) { // > 1MB
            Toast.makeText(
                context,
                context.getString(R.string.msg_move_started, destination.name),
                Toast.LENGTH_LONG
            ).show()
        } else {
            Toast.makeText(
        ...
        scope.launch {
            ...
            val progressDialog = FileOperationProgressDialog.show(
                context,
                context.getString(R.string.moving_files),
                onCancel = { 
            ...
            try {
                    ...
                    fileOperationUseCase.executeWithProgress(operation).collect { progress ->
                        ...
                        withContext(Dispatchers.Main) {
                            ...
                            if (progress is com.sza.fastmediasorter.domain.usecase.FileOperationProgress.Completed) {
                                ...
                                handleMoveResult(progress.result, destinationFolder, destination)
                            }
                        }
                    }
                }
                ...
                withContext(Dispatchers.Main) {
                    Toast.makeText(context, R.string.toast_move_cancelled, Toast.LENGTH_SHORT).show()
                ...
                withContext(Dispatchers.Main) {
                    com.sza.fastmediasorter.ui.dialog.ErrorDialog.show(
                        context,
                        context.getString(R.string.error_move),
