...
class DropboxClient @Inject constructor(
    ...
    companion object {
        private const val APP_NAME = "FastMediaSorter/2.6"
     ...
     * Uses BuildConfig.DROPBOX_KEY if not present in JSON
     */
    private fun deserializeCredential(json: String): DbxCredential? {
        return try {
            ...
            // Use stored app_key if available, otherwise fallback to BuildConfig
            val appKey = obj.optString("app_key", null) ?: BuildConfig.DROPBOX_KEY
