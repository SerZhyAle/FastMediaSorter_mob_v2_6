...
class FtpOperationStrategy @Inject constructor(
    ...
    /**
     * Connection configuration for FTP operations.
     */
    ...
    // Connection config for current FTP session
    ...
    fun configure(
        ...
        config = FtpConfig(host, port, username, password)
    }
    ...
    override suspend fun move(
        ...
        if (isRemotePath(source.path) && isRemotePath(destinationPath)) {
            ...
            val renameResult = ftpClient.rename(
                cfg.host, cfg.port, sourceRemote, destRemote, cfg.username, cfg.password
            )
    ...
    override suspend fun delete(
        ...
        try {
            ...
            val deleteResult = ftpClient.deleteFile(
                cfg.host, cfg.port, remotePath, cfg.username, cfg.password
            )
    ...
    override suspend fun rename(
        ...
        try {
            ...
            val renameResult = ftpClient.rename(
                cfg.host, cfg.port, remotePath, newRemotePath, cfg.username, cfg.password
            )
    ...
    override suspend fun exists(path: String): Boolean = withContext(Dispatchers.IO) {
        ...
        try {
            ...
            val existsResult = ftpClient.exists(
                cfg.host, cfg.port, remotePath, cfg.username, cfg.password
            )
    ...
    override suspend fun createDirectory(path: String): Result<Unit> = withContext(Dispatchers.IO) {
        ...
        try {
            ...
            val mkdirResult = ftpClient.createDirectory(
                cfg.host, cfg.port, remotePath, cfg.username, cfg.password
            )
    ...
    override suspend fun getFileInfo(path: String): Result<MediaFile> = withContext(Dispatchers.IO) {
        ...
        try {
            ...
            // Get file size
            val sizeResult = ftpClient.getFileSize(
                cfg.host, cfg.port, remotePath, cfg.username, cfg.password
            )
    ...
    private fun isRemotePath(path: String): Boolean {
        return path.startsWith(FTP_SCHEME)
    }

    private fun extractRemotePath(fullPath: String): String {
        // ftp://host:port/path -> /path
        if (!fullPath.startsWith(FTP_SCHEME)) return fullPath
    ...
    private suspend fun downloadFile(
        ...
        FileOutputStream(localFile).use { outputStream ->
            val result = ftpClient.downloadFile(
                cfg.host, cfg.port, remoteFilePath, outputStream, cfg.username, cfg.password
            )
    ...
    private suspend fun uploadFile(
        ...
        FileInputStream(localFile).use { inputStream ->
            val result = ftpClient.uploadFile(
                cfg.host, cfg.port, remoteFilePath, inputStream, cfg.username, cfg.password
            )
